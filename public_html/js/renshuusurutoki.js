...
var renshuuMain = {
	...
	ready: function () {
		...
		*/
		...
		if (location.hash !== '') {
			...
			if (key == 'filters' || renshuuMain.forms.types.indexOf(key) !== -1) {
				renshuuMain.showTabContent(key);
			}
		}
		else {
	...
	showTabContent: function (key) {
		...
		if (renshuuMain.forms.types.indexOf(key) !== -1) {
			renshuuMain.forms.getForm(key);
		}
...
renshuuMain.forms = {
	...
	getForm: function (type) {
		if (renshuuMain.forms.cache[type])
		{
			renshuuMain.forms.showForm(type);
		}
		else
		{
			$.post('/ajax/form/' + type, function (data, status) {
				...
				{
					renshuuMain.forms.setForm(data.response.form, type);
					renshuuMain.forms.showForm(type);
				}
			}, 'json');
		}
	...
	setForm: function (form, type) {
		...
		*/
		...
		renshuuMain.forms.cache[type] = form;
	},
	...
	showForm: function (type) {
		var form = renshuuMain.forms.cache[type];
		...
		if (type == 'location' && localStorage.getItem('locationGeocode')) {
			...
			renshuuMain.forms.fillUserData();
		}
	},
